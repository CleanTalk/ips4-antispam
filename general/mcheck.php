...
$html.="</tbody></table></center><br/><center><b><a href=\"#\" id=\"ct_delete_checked\" onclick=\"var answer = confirm('Are you sure you want to delete these users? All messages and topics from this users also will be deleted!'); if (answer) $.ajax({type: 'POST', url: 'http://".$_SERVER['SERVER_NAME']."/uploads/updatedb.php', data: ''});\">Delete selected</a></b><span style='padding-left:10px;'> </span> <b><a href=\"#\" id=\"ct_delete_all\" onclick=\"var answer = confirm('Are you sure you want to delete these users? All messages and topics from this users also will be deleted!'); if (answer){ $.ajax({type: 'POST', url: 'http://".$_SERVER['SERVER_NAME']."/uploads/updatedb.php', data: '" . http_build_query(array('spam_users' => $spam_array)) . "', success: function(data){ $('#delete_results').html(data);}});}\">Delete all</a></b></center><br /><div id = 'delete_results'></div>";
}
else
{
