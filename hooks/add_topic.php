...
abstract class antispambycleantalk_hook_add_topic extends _HOOK_CLASS_
{
	...
	public function add()
	{
		try{
			...
			$topic_title = isset($_POST['topic_title']) ? trim(strip_tags($_POST['topic_title'])) : '';
			$topic_guest_name = isset($_POST['guest_name']) ? trim(strip_tags($_POST['guest_name'])) : '';
			$topic_content = isset($_POST['topic_content']) ? trim(strip_tags($_POST['topic_content'])) : '';

			// If empty required data - return
			if (!$topic_title || !$topic_content) {
				return parent::add();
			}

			$topic_message = $topic_title . "\n" . $topic_guest_name . "\n" . $topic_content;
			$lang = \IPS\Lang::getEnabledLanguages();
			$locale = $lang[\IPS\Lang::defaultLanguage()]->short;

			// Pointer data
			$pointer_data = isset($_COOKIE['ct_pointer_data']) ? json_decode($_COOKIE['ct_pointer_data'], true) : 0;
			// Timezone from JS
			...
			$ct_request->sender_nickname = $topic_guest_name;
